...
while any(tosplit),
    ...
    for ig=ug(tosplit(ug)),
        ...
        if ng>2
            ...
            if FLAG, disp(['WARNING: EIGS did not converge!',' ng: ', num2str(ng), ' ?: ',num2str(3)]); end
            D=diag(D); [D,iord]=sort(D,'descend'); V=V(:,iord);
        else
            [V,D,FLAG] = eigs(B,ng,2,'la',opts,A,indx);
            if FLAG, disp(['WARNING: EIGS did not converge!',' ng: ', num2str(ng), ' ?: ',num2str(2)]); end
            D=diag(D); [D,iord]=sort(D,'descend'); V=V(:,iord);
...
while (Q>Q0), %Outer loop until improvement not found inside
    %%disp("number of iters : modularity Q")
...
%disp([int2str(numiter),':  Stopped because best Q = ',num2str(bestQ)])
Q=modularity(groups,B,A,1:N);
%end kln%
